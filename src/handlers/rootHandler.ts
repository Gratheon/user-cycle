...
export function rootHandler(request, reply) {
    const endpoints = [
        ...
        { method: 'GET', path: '/account/cancel', description: 'Account cancellation redirect' },
        { method: 'GET', path: '/graphql', description: 'GraphQL playground' },
        // Stripe endpoints
        { method: 'POST', path: '/stripe/webhook', description: 'Stripe webhook handler' },
        { method: 'GET', path: '/stripe/success', description: 'Stripe success callback' },
        { method: 'GET', path: '/stripe/cancel', description: 'Stripe cancellation callback' },
        // Google endpoints
        { method: 'GET', path: '/google/auth', description: 'Google authentication' },
        { method: 'GET', path: '/google/callback', description: 'Google authentication callback' }
    ];
...
<svg viewBox="0 0 500 147" xmlns="http://www.w3.org/2000/svg">
  ...
  <path d="M 412.585 62.353 L 455.59 107.297 L 455.68 66.226 L 464.378 66.279 L 464.349 128.492 C 464.349 128.492 421.335 83.531 421.382 83.862 L 421.342 125.014 L 412.524 124.947 L 412.585 62.353 Z" style="stroke: rgb(255, 255, 255); stroke-width: 0px;"/>
    ...
    <html>
    <head>
        <title>User-Cycle Service API</title>
        <style>
    ...
    <body>
        <div class="header" style="display: flex; flex-direction: row; align-items: center;">
            ...
            <h1>User-Cycle microservice API</h1>
        </div>
        <p>This service handles user authentication, billing, and translations for Gratheon.com</p>
